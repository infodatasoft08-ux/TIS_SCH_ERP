...
export default function StudentAttendanceCalendar() {
  ...
  const stats = useMemo(() => {
    ...
    Object.values(recordsMap).forEach(rec => {
      ...
      else if (st === 'excused' || st === 'leave') excused++;
    });

    const total = present + absent + late + excused;
    const ratio = total > 0 ? Math.round((present / total) * 100) : 100;

    return { present, absent, late, excused, total, ratio };
  }, [recordsMap]);
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto p-2 sm:p-6 pb-28 sm:pb-12 animate-in fade-in duration-500 select-none">
      ...
      {/* Top Header & Attendance Performance Card */}
      <div className="bg-gradient-to-r from-slate-900 via-blue-950 to-indigo-950 text-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xl border border-white/10 relative overflow-hidden">
        <div className="absolute top-0 right-0 -mr-12 -mt-12 w-64 h-64 rounded-full bg-blue-500/20 blur-3xl pointer-events-none" />
        
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/15 backdrop-blur-md text-[11px] font-semibold text-blue-200 mb-2 border border-white/15">
            ...
            <p className="text-xs sm:text-sm text-blue-200/80 mt-1 max-w-md">
              Track your daily presence, leaves, and overall performance score.
            </p>
          </div>

          {/* Attendance Ratio Gauge */}
          <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md p-3 rounded-2xl border border-white/15 shrink-0 self-stretch sm:self-auto justify-between sm:justify-start">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-wider text-blue-200">Monthly Performance</div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 flex-1 max-w-xl">
            ...
            <Button
              ...
              disabled={loading}
              className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold text-xs rounded-xl px-4 py-2 shadow-md transition-all"
            >
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 pb-3 sm:pb-4 mb-3 sm:mb-4 border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Button
              ...
              size="icon"
              onClick={prevMonth}
              className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl border-gray-200 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-gray-800"
            >
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  dayItem.isToday && "ring-2 ring-blue-500 bg-blue-50/30 dark:bg-blue-950/20",
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span className={cn(
                    ...
                    dayItem.isToday && "w-5 h-5 sm:w-auto sm:h-auto bg-blue-600 text-white dark:text-white font-black text-[11px] sm:text-sm sm:bg-transparent sm:text-blue-600 dark:sm:text-blue-400"
                  )}>
                    {dayItem.dayNumber}
                  </span>

                  {dayItem.isToday && (
                    <span className="hidden sm:inline-block text-[9px] font-bold px-1.5 py-0.5 rounded bg-blue-600 text-white uppercase tracking-wider">
                      Today
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-2.5 bg-gray-50 dark:bg-gray-800/50 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 text-xs sm:text-sm">
                  <div className="flex justify-between items-center py-1 border-b border-gray-200/50 dark:border-gray-700/50">
                    ...
                    <span className="font-bold text-gray-900 dark:text-white">
                      {selectedDayRecord.student_name || user?.name || "N/A"}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center py-1 border-b border-gray-200/50 dark:border-gray-700/50">
                    <span className="text-gray-500 dark:text-gray-400 flex items-center gap-1.5 font-medium">
                      <Building className="w-4 h-4 text-purple-500" /> Class:
                    </span>
                    <span className="font-bold text-gray-900 dark:text-white">
                      {selectedDayRecord.class_name || "N/A"}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center py-1">
                    ...
                    <span className="font-bold text-gray-900 dark:text-white">
                      {selectedDayRecord.taken_by || selectedDayRecord.Taken_By || "System"}
                    </span>
                  </div>
                </div>
