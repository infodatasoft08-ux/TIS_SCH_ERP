...
import { BookOpen, Calendar, Code, GraduationCap, LayoutGrid, List } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function ViewSubjectAssignToTeacher() {
    ...
    const fetchSubjects = async () => {
        try {
            setLoading(true);
            // Endpoint from backend/routes/teachersOperation.js line 43
            const res = await API.get("/teachers/get/teacher/subjects");
    ...
    if (error) {
        return (
            <div className="p-8 text-center bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-900 mx-auto max-w-lg mt-10">
                ...
                <Button onClick={fetchSubjects} variant="outline" className="border-red-300 hover:bg-red-100 dark:border-red-800 dark:hover:bg-red-900/50">
                    Try Again
                </Button>
            </div>
        );
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-1000 max-w-7xl mx-auto p-4 md:p-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border pb-6">
                <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground text-sm md:text-base">
                        Manage and view details of all subjects assigned to you for this academic year.
                    </p>
                </div>
            ...
            {/* Stats Overview - Optional, only show if we have data */}
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-muted/20 rounded-2xl border-2 border-dashed border-muted">
                    ...
                    <p className="text-muted-foreground max-w-sm mx-auto">
                        You currently don't have any subjects assigned. Please contact the administrator if you believe this is an mistake.
                    </p>
                </div>
            ) : (
                <div className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "space-y-4"}>
                    {subjects.map((subject, index) => (
                        viewMode === "grid" ? (
                            <Card
                                key={subject.id}
                                className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-primary/50 dark:hover:border-primary/50 border-muted/60"
                            >
                                {/* Image Header */}
                                <div className="relative h-48 overflow-hidden">
                                    ...
                                    <img
                                        ...
                                        onError={(e) => { if (subject.image_url) e.target.src = getSubjectImage(index); }}
                                    />
                                    <div className="absolute bottom-4 left-4 right-4 z-20">
                                        <div className="flex justify-between items-end">
                                            <div>
                                                ...
                                                <h3 className="text-xl font-bold text-white leading-tight shadow-sm line-clamp-2">
                                                    {subject.name}
                                                </h3>
                                            </div>
                                        </div>
                                ...
                                <CardContent className="p-5 space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3 pt-2">
                                        <div className="bg-muted/30 p-2.5 rounded-lg border border-border/50 flex flex-col items-center justify-center text-center gap-1 group/item hover:bg-primary/5 transition-colors">
                                        ...
                                        <div className="bg-muted/30 p-2.5 rounded-lg border border-border/50 flex flex-col items-center justify-center text-center gap-1 group/item hover:bg-primary/5 transition-colors">
                                            ...
                                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Classes</span>
                                            <span className="text-sm font-bold text-foreground">Weekly</span>
                                        </div>
                                    </div>

                                    <Button className="w-full mt-2 group-hover:translate-y-0 transition-all font-medium" variant="outline">
                                        View Materials
                                    </Button>
                                </CardContent>
                            </Card>
